...
public class Order : AggregateRoot<Guid>
{
    ...
    public void RequestCashPayment()
    {
        EnsureNotCancelled();
        if (OrderStatus == OrderStatus.Draft)
            throw new InvalidOperationException("Chưa gửi đơn, không thể yêu cầu thanh toán.");
        if (OrderStatus == OrderStatus.Paid) return; // already paid
    ...
    {
        EnsureNotCancelled();
        if (OrderStatus == OrderStatus.Paid) return;
        OrderStatus = OrderStatus.Paid;
        PaidAtUtc = DateTime.UtcNow;
    }
    ...
    private void EnsureNotCancelled()
    {
        if (OrderStatus == OrderStatus.Cancelled)
            throw new InvalidOperationException("Đơn đã huỷ.");
    }
